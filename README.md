...
- `git issue ls [<status>|--all|--open|--closed] [--porcelain] [--sort date|priority] [--priority-gt N] [--priority-lt N] [--priority N] [--tag T]`
  - Lists issues (`refs/issues/*`) filtered by status; default shows open issues. Displays short hash and title, plus any non-default priority (`[P:N]`) and tags (`[tag1, tag2]`).
  - `--all` shows every status; `--open`/`--closed` filter to those values; any other positional `<status>` string filters to that exact value.
  - `--porcelain` emits machine-readable `<full-hash>|<status>|<title>|<priority>` lines.
  ...
  - `--tag T` filters to issues whose tags contain `T` (can be combined with status/priority filters). Use `--tag '!T'` to match issues that do NOT have tag `T`.
  - Examples:
    - `git issue ls` (open)
    - `git issue ls closed`
    - `git issue ls --all`
...
- `index.html` lists **open** issues (sorted by earliest message timestamp), with a link to the closed-issues page and the tags index.
- `closed.html` lists **closed** issues, with a link back to the open-issues page.
- Each issue page shows title, status, priority (when set), tags, opener, timestamps, and all messages with authors.
- `tags.html` lists every tag, and `tags/<tag>.html` lists the issues carrying that tag (tags are linked from issue and list pages).
